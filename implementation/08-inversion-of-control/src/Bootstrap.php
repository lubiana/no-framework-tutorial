...
try {
    switch ($routeInfo[0]) {
        ...
        case Dispatcher::FOUND:
            ...
            $handler = new $routeInfo[1][0]($response);
    ...
    $response = (new NotAllowed($response))->handle($request);
} catch (NotFoundException) {
    $response = (new NotFound($response))->handle($request);
} catch (Exception) {
    $response = (new InternalServerError($response))->handle($request);
}
